...
class ResNetEmbedding(torch.nn.Module):
    def __init__(self, embedding_size, normalize_embeddings=False):
        super().__init__()
        ...
        self.backbone = torch.nn.Sequential(*layers_)

        # Encoder
        self.encoder = torch.nn.Linear(num_ftrs, embedding_size)
