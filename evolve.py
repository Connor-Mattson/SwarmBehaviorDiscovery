...
flags.DEFINE_bool("species_aware", False, "Whether to evaluate features at the subgroup level and concatenate subgroup behaviors into one vector for novelty search")
...
flags.DEFINE_string("custom_name", None, "Custom Experiment Name")
...
def main(_):
    ...
    if FLAGS.custom_name:
        name = f"{FLAGS.custom_name}_s{FLAGS.seed}_t{int(time.time())}"
    else:
        name = f"{FLAGS.strategy}_concat{FLAGS.species_aware}_{FLAGS.type}_g{gen}_p{pop}_t{int(time.time())}"

    # Human-in-the-loop Training
    try:
        evolve_and_cluster(name, FLAGS.type, FLAGS.checkpoint, gen, pop, cr, mr, FLAGS.k, FLAGS.seed, FLAGS.agents, FLAGS.lifespan,
                           heterogeneous=heterogeneous, concat=FLAGS.concat, export_medoids=FLAGS.exp_medoids, evaluate_sub_groups=FLAGS.species_aware, strategy=FLAGS.strategy)
